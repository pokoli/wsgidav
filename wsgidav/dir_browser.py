...
"""
WSGI middleware that handles GET requests on collections to display
directories.
...
msOfficeTypeToExtMap = {
    ...
    "powerpoint": ("pps", "ppt", "pptm", "pptx", "potm", "potx", "ppsm",
        "ppsx"),
...
function openWithSharePointPlugin(url) {
    ...
    if( window.ActiveXObject ){
        try {
            // Office 2007
            control = new ActiveXObject("SharePoint.OpenDocuments.3");
        } catch(e) {
            try {
                // Office 2003
                control = new ActiveXObject("SharePoint.OpenDocuments.2");
            } catch(e2) {
                try {
                    // Office 2000/XP
                    control = new ActiveXObject("SharePoint.OpenDocuments.1");
                } catch(e3) {
...
class WsgiDavDirBrowser(BaseMiddleware):
    ...
    def __call__(self, environ, start_response):
        ...
        if environ["wsgidav.provider"]:
            davres = environ["wsgidav.provider"].getResourceInst(
                path, environ)

        if (environ["REQUEST_METHOD"] in ("GET", "HEAD") and davres
                and davres.isCollection):
            ...
            #     util.status("Directory browsing disabled for agent '%s'" %
            ...
            if (dirConfig.get("davmount")
                    and "davmount" in environ.get("QUERY_STRING")):
...
#                profile.print_stats(sort=2)
    ...
    def isSuitable(config):
        return (config.get("dir_browser") and
            config["dir_browser"].get("enable", True))
    ...
    def _listDirectory(self, davres, environ, start_response):
        ...
        if trailer:
            trailer = trailer.replace("${version}",
              "<a href='https://github.com/mar10/wsgidav/'>WsgiDAV/%s</a>" %
                __version__)
            trailer = trailer.replace("${time}", util.getRfc1123Time())
        else:
            trailer = (
                ("<a href='https://github.com/mar10/wsgidav/'>WsgiDAV/%s</a>"
                    "- %s" % (__version__, util.getRfc1123Time())))
        ...
        if dirConfig.get("ms_mount"):
            html.append(
                "<style type='text/css'> A {behavior: "
                "url(#default#AnchorClick);} </style>")

        if dirConfig.get("ms_sharepoint_plugin"):
            html.append(
                "<object id='winFirefoxPlugin'"
                "type='application/x-sharepoint' width='0' height='0'"
        ...
        if dirConfig.get("davmount"):
            links.append("<a title='Open this folder in a WebDAV client.' "
                "href='%s?davmount'>Mount</a>" %
                util.makeCompleteUrl(environ))
        if dirConfig.get("ms_mount"):
            links.append("<a title='Open as Web Folder (requires Microsoft "
                "Internet Explorer)' href='' FOLDER='%s'>Open as Web Folder"
                "</a>" % util.makeCompleteUrl(environ))
#                html.append("<a href='' FOLDER='%ssetup.py'>Open setup.py as"
#                        "WebDAV</a>" % util.makeCompleteUrl(environ))
        if links:
        ...
        if davres.path in ("", "/"):
            html.append(
                "<tr><td>Top level share</td> <td></td> <td></td> <td></td>"
                "</tr>")
        else:
            parentUrl = util.getUriParent(davres.getHref())
            html.append("<tr><td><a href='" + parentUrl +
                "'>Parent Directory</a></td> <td></td> <td></td> <td></td>"
                "</tr>")
        ...
        for infoDict in dirInfoList:
            ...
            html.append("""\
            <tr><td><a href="%(href)s" class="%(class)s">%(displayName)s</a>"
                "</td>"
            <td>%(displayType)s</td>
