...
class SphinxCommand(Command):
    ...
    def run(self):
        ...
        res = subprocess.call("sphinx-build -b html doc/sphinx "
            "doc/sphinx-build", shell=True)
        if res:
            print("ERROR: sphinx-build exited with code {}".format(res))
        else:
            print("Documentation created at {}.".format(
...
if "HOME" not in os.environ and "HOMEPATH" in os.environ:
    ...
    print("Initializing HOME environment variable to '%s'" %
        os.environ["HOME"])
...
try:
    ...
    print("Could not import cx_Freeze; 'build' and 'bdist' commands will not "
        "be available.")
